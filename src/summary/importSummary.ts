...
const MB = 1024 * 1024;
export const LIMITS = { pdf: 50 * MB, html: 10 * MB, assets: 40 * MB };
...
export const SUMMARY_ACCEPT = ['.pdf', '.html', '.htm', ...Object.keys(ASSET_MIME).map((e) => `.${e}`)].join(',');
...
export interface SummaryInputFile {
  name: string;
  /** Path as referenced by the HTML (defaults to the name). */
  path?: string;
  blob: Blob;
}
 ...
 * Accepts exactly one PDF or HTML file; for HTML, any additional images/CSS/fonts are
 * kept as assets so the page can be displayed completely offline.
...
export async function buildSummary(files: SummaryInputFile[]): Promise<ParsedSummary> {
  const primaries = files.filter((f) => ['pdf', 'html', 'htm'].includes(extension(f.name)));
  ...
  const text = decodeHtml(new Uint8Array(await main.blob.arrayBuffer()));
